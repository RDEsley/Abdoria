...
export function buildLocalNotices(stats: DashboardStats | null): AppNotification[] {
  ...
  if (stats.frozen_streak_count === 1) {
    notices.push({
      ...
      corpo: 'Depois dele, um dia sem treino zera a sequência. A Exploração AFK dropa mais.',
      payload: {},
      lida_em: now,
      criada_em: now,
    });
    ...
    notices.push({
      ...
      corpo: `Seus ${stats.streak_atual} dias de sequência estão sem Frozen Streak de reserva — a Exploração AFK é a melhor fonte.`,
      payload: {},
      lida_em: now,
      criada_em: now,
    });
