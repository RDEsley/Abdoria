...
export function buildLocalNotices(
  ...
  if (stats.frozen_streak_count === 1) {
    notices.push({
      ...
      corpo: 'Depois dele, um dia sem treino ou atividade pode zerar a sequência.',
      payload: {},
      lida_em: now,
      criada_em: now,
    });
    ...
    notices.push({
      ...
      corpo: `Seus ${stats.streak_atual} dias de sequência estão sem Frozen Streak de reserva.`,
      payload: {},
      lida_em: now,
      criada_em: now,
    });
