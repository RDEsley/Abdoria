...
describe('listReminderOccurrencesInLookback', () => {
  ...
  it('suporta múltiplos horários no mesmo dia', () => {
    ...
    const occurrences = listReminderOccurrencesInLookback([reminder], now, 'America/Sao_Paulo', 15);
  ...
  it('preserva lembrete único (once) dentro da janela', () => {
    ...
    const occurrences = listReminderOccurrencesInLookback([once], now, 'America/Sao_Paulo', 15);
    expect(occurrences).toHaveLength(1);
  ...
  it('respeita timezone da subscription', () => {
    ...
    const sp = listReminderOccurrencesInLookback([reminder], now, 'America/Sao_Paulo', 10);
...
describe('decideDeliveryClaim', () => {
  ...
  it('não reenvia ocorrência já marcada como sent', () => {
    expect(
      decideDeliveryClaim({ status: 'sent', attempts: 1, updatedAtMs: now - 60_000 }, now),
    ).toBe('skip');
  });

  it('permite retry após falha transitória respeitando cooldown', () => {
    expect(
      decideDeliveryClaim({ status: 'failed', attempts: 1, updatedAtMs: now - 120_000 }, now),
    ).toBe('retry');
    expect(
      decideDeliveryClaim({ status: 'failed', attempts: 1, updatedAtMs: now - 10_000 }, now),
    ).toBe('skip');
  });

  it('cada subscription decide de forma independente (estado isolado)', () => {
    const subA = decideDeliveryClaim({ status: 'sent', attempts: 1, updatedAtMs: now }, now);
  ...
  it('para após atingir o máximo de tentativas', () => {
    expect(
      decideDeliveryClaim({ status: 'failed', attempts: 5, updatedAtMs: now - 120_000 }, now),
    ).toBe('skip');
  });
});
...
describe('reminder push config', () => {
  ...
  it('falha claramente sem VAPID_SUBJECT', () => {
    ...
    expect(() => assertReminderPushConfigured()).toThrow(ReminderPushMisconfiguredError);
