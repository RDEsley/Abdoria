...
export const PushSubscriptions = {
  ...
  async listAll(): Promise<PushSubscriptionRow[]> {
    const sb = getSupabase();
    const { data, error } = await sb.from('push_subscriptions').select('*').order('id');
    if (error) throw error;
...
export const PushDeliveryLog = {
  async claim(
    ...
    const decision = decideDeliveryClaim(toClaimState(existing as DeliveryRow), nowMs);
  ...
  async markSent(subscriptionId: string, occurrenceKey: string, now = new Date()): Promise<void> {
    const sb = getSupabase();
    const { error } = await sb
      .from('push_delivery_log')
      .update({
  ...
  async pruneOlderThan(days: number): Promise<void> {
    ...
    const { error } = await sb.from('push_delivery_log').delete().lt('atualizada_em', cutoff);
    if (error) throw error;
  },
};
